import argparse

import gradio as gr
from peft import AutoPeftModelForCausalLM, PeftModel
import torch
from transformers import AutoModelForCausalLM, AutoTokenizer, TextIteratorStreamer
from threading import Thread

from langchain.vectorstores.pgvector import PGVector
from langchain.embeddings.sentence_transformer import SentenceTransformerEmbeddings
import re


#------------------------------FUNCTIONS------------------------------#
def concat_RAG_page_content(docs_input):
    """
    Concatentate content from multiple documents in RAG
    """
    
    concat_docs = "\n".join([tup[0].page_content for tup in docs_input])
    
    return concat_docs

def concat_RAG_sources_and_pages(docs_input):
    """
    Concatenate sorted list of sources+pages for multiple documents 
    retrieved for RAG
    """
    
    concat_source_lst = []
    for tup in docs_input:
        doc_source = re.search('(MSL .{1,})', tup[0].metadata["source"])[0]
        doc_page = tup[0].metadata["page"]
        
        source_page = f"{doc_source}, page = {doc_page}"
        concat_source_lst.append(source_page)
        
    concat_source_lst.sort()        
    concat_source_info = "; ".join(concat_source_lst)
    
    return concat_source_info
#------------------------------SCRIPT------------------------------#
parser = argparse.ArgumentParser()
parser.add_argument(
    "--model_path_or_id",
    type=str,
    default="mistralai/Mistral-7B-v0.1",
    required=False,
    help="Model ID or path to saved model",
)

parser.add_argument(
    "--lora_path",
    type=str,
    # default=None,
    # default="../III_Finetuning_For_RAG/mistral-7b-int4-dolly_summarization_r8_a16_ep1_LR1e3_qkvo_fixedtokens", #AP: ADDED
    default="../III_Finetuning_For_RAG//mistral-7b-int4-dolly_summarization_r8_a16_ep1_LR1e3_allproj_fixedtokens",
    required=False, # AP: Changed to True
    help="Path to the saved lora adapter",
)

args = parser.parse_args()

if args.lora_path:
    # load base LLM model with PEFT Adapter
    model = AutoPeftModelForCausalLM.from_pretrained(
        args.lora_path,
        low_cpu_mem_usage=True,
        torch_dtype=torch.float16,
        bnb_4bit_compute_dtype=torch.float16,
        use_flash_attention_2=True,
        load_in_4bit=True,
    )
    tokenizer = AutoTokenizer.from_pretrained(args.lora_path, padding_side="right") #AP: Added padding side
else:
    model = AutoModelForCausalLM.from_pretrained(
        args.model_path_or_id,
        low_cpu_mem_usage=True,
        torch_dtype=torch.float16,
        bnb_4bit_compute_dtype=torch.float16,
        use_flash_attention_2=True,
        load_in_4bit=True,
    )
    tokenizer = AutoTokenizer.from_pretrained(args.model_path_or_id)

# The connection to the database
CONNECTION_STRING = PGVector.connection_string_from_db_params(
    driver= "psycopg2",
    host = "localhost",
    port = "5432",
    database = "postgres",
    user= "username",
    password="password"
)

# The embedding function that will be used to store into the database
embedding_function = SentenceTransformerEmbeddings(
    model_name="BAAI/bge-large-en-v1.5",
    model_kwargs = {'device': 'cuda'},
    encode_kwargs = {'normalize_embeddings': True}
)

# Creates the database connection to our existing DB
db = PGVector(
    connection_string = CONNECTION_STRING,
    collection_name = "embeddings",
    embedding_function = embedding_function
)


# AP: ADDED
RAG_PROMPT_TEMPLATE = """### Context:
{context}

### Question:
Using only the context above, {question}

### Response:
"""

with gr.Blocks() as demo:
    gr.HTML(
        f"""
        <h2> Instruction Chat Bot Demo </h2>
        <h3> Model ID : {args.model_path_or_id} </h3>
        <h3> Peft Adapter : {args.lora_path} </h3>
    """
    )

    chat_history = gr.Chatbot(label="QA Bot")
    msg = gr.Textbox(label="Question")
    with gr.Accordion(label="Generation Parameters", open=False):
        prompt_format = gr.Textbox(
            label="Formatting prompt", 
            # value="{question}", 
            value=RAG_PROMPT_TEMPLATE,
            lines=8
            
        )
        with gr.Row():
            max_new_tokens = gr.Number(
                minimum=25, maximum=500, value=100, label="Max New Tokens"
            )
            temperature = gr.Slider(
                minimum=0, maximum=1.0, value=0.1, label="Temperature"
            )

    clear = gr.ClearButton([msg, chat_history])

    def user(user_message, history):
        return "", [[user_message, None]]

    def bot(chat_history, prompt_format, max_new_tokens, temperature):
        # Format the instruction using the format string with key
        # {instruction}

        # docs_with_scores = db.similarity_search_with_score(chat_history[-1][0], k = 1)
        # docs_with_scores = db.similarity_search_with_score(chat_history[-1][0], k = 2) # AP: Pull 2 documents
        docs_with_scores = db.similarity_search_with_score(chat_history[-1][0], k = 4) # AP: Pull 2 documents
        formatted_inst = prompt_format.format(
            # context = docs_with_scores[0][0].page_content,
            context = concat_RAG_page_content(docs_with_scores),
            question = chat_history[-1][0]
        )

        # Tokenize the input
        input_ids = tokenizer(
            formatted_inst, return_tensors="pt", truncation=True
        ).input_ids.cuda()

        # Support for streaming of tokens within generate requires
        # generation to run in a separate thread
        streamer = TextIteratorStreamer(tokenizer, skip_prompt=True)
        generation_kwargs = dict(
            input_ids=input_ids,
            streamer=streamer,
            max_new_tokens=max_new_tokens,
            do_sample=True,
            top_p=0.9,
            temperature=temperature,
            use_cache=True,
        )

        thread = Thread(target=model.generate, kwargs=generation_kwargs)
        thread.start()
        chat_history[-1][1] = ""
        for new_text in streamer:
            chat_history[-1][1] += new_text
            yield chat_history
            
        
        if docs_with_scores[0][1] > 0.55:
            for i in range(1):
                disclaimer_output = "Information did not seem likely to be in available documents. Answer provided from prior knowledge."
                chat_history[-1][1] += disclaimer_output
                yield chat_history
        else:
            for i in range(1):
                source_string = concat_RAG_sources_and_pages(docs_with_scores)
                chat_history[-1][1] += f"\nSource(s): {source_string}" #AP: ADDED
                yield chat_history

    msg.submit(user, [msg, chat_history], [msg, chat_history], queue=False).then(
        bot, [chat_history, prompt_format, max_new_tokens, temperature], chat_history
    )

demo.queue()
demo.launch()
